...
@Component({
  ...
  animations: [
    trigger('bloat', [
      ...
      state('big', style({
        transform: 'scale(1.5) translate(-50%, -5%)',
        opacity: 1.0
      })),
      state('small', style({
        transform: 'scale(1.0) translate(0, 0)',
      ...
      transition('* => init', [
        animate('2.5s ease-out', keyframes([
          ...
          style({ transform : 'scale(1.5) translate(-50%, 0%)', opacity: 1.0 }),
...
export class LogoComponent implements OnInit {
  ...
  @Input() textColor: string = '#cccccc'
  @Input() dotColor: string = '#373334'

  private animationState: 'init' | 'big' | 'small' = 'init'
