...
import {
  ...
  GallerySpaceModel, GallerySpace,
  ExhibitionModel, Exhibition,
  FrontPageImageModel, FrontPageImage,
  PostModel, Post,
  FairModel, Fair,
  ArtworkSeries, ArtworkSeriesModel
} from '../models'
...
export class ApiService {
  ...
  initControllers() {
    this.frontPageImages = new Controller<FrontPageImageModel, FrontPageImage>(
      this, '/front-page-images', FrontPageImage
    )
    this.artists = new Controller<ArtistModel, Artist>(
      this, '/artists', Artist
    )
    this.exhibitions = new Controller<ExhibitionModel, Exhibition>(
      this, '/exhibitions', Exhibition
    )
    this.news = new Controller<PostModel, Post>(
      this, '/blog/news', Post
    )
    this.gallerySpaces = new Controller<GallerySpaceModel, GallerySpace>(
      this, '/gallery-spaces', GallerySpace
    )
    this.fairs = new Controller<FairModel, Fair>(
      this, '/fairs', Fair
    )
    ...
    this.artworkSeries = new Controller<ArtworkSeriesModel, ArtworkSeries>(
      this, '/artwork-series', ArtworkSeries
    )
  }
...
class Controller<M, T extends Model> {
  ...
  constructor(private api: ApiService, private apiRoute: String,
    private c : new (api: ApiService, m) => T
  ) {
    this.api.lang.languageChanged.subscribe(
      event => {
  ...
  private fetch () : Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      this.api.http.get(apiPrefix + this.apiRoute)
        .subscribe(
          (response: M[]) => {
  ...
  public withArgs(id: string): Observable<T[]> {
    return this.api.http.get(apiPrefix + this.apiRoute + '/' + id)
      .pipe(map((response: M[]) => 
        response.map(data => {
