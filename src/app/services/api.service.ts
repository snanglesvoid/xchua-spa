...
import {ViewingRoomModel, ViewingRoom} from '../models/ViewingRoom';
...
export class ApiService {
  ...
  public viewingRoom: Controller<ViewingRoomModel, ViewingRoom>;
  public viewingRooms: Controller<ViewingRoomModel, ViewingRoom>;
  ...
  initControllers() {
    ...
    this.viewingRooms = new Controller<ViewingRoomModel, ViewingRoom>(
      this,
      '/viewing-rooms',
      ViewingRoom
    );
    this.viewingRoom = new Controller<ViewingRoomModel, ViewingRoom>(
      this,
      '/viewing-room',
      ViewingRoom
    );
...
class Controller<M, T extends Model> {
  ...
  constructor(
    ...
    private c: new (api: ApiService, m: any) => T
  ) {
    this.api.lang.languageChanged.subscribe((event: any) => {
    ...
    return new Promise((resolve, reject) => {
      this.api.http.get(apiPrefix + this.apiRoute).subscribe(
        (response: M[]) => {
          ...
          const event = 'controller data loaded: ' + this.c.name;
  ...
  public async waitForData(): Promise<T[]> {
    if (this.data && this.data.length > 0) {
      ...
      return new Promise((resolve, _) => {
        this.loaded.subscribe(() => resolve(this._data));
      });
    }
  }
  ...
  public withArgs(id: string): Observable<T[]> {
    return this.api.http.get(apiPrefix + this.apiRoute + '/' + id).pipe(
      map((response: M[]) =>
        response.map(data => {
          const t = new this.c(this.api, data);
