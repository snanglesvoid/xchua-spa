...
export class Exhibition extends Model {
    ...
    constructor(api: ApiService, private model: ExhibitionModel) {
        ...
        if (typeof model.location === 'string') {
            ...
            this._location = new GallerySpace(api, model.location as GallerySpaceModel)
        }
        if (model.artists && model.artists.length && typeof model.artists[0] === 'string') {
            this._artists = model.artists as string[]
        }
        else {
            this._artists = model.artists.map(x => new Artist(api, x as ArtistModel))
        }
        if (model.artworks && model.artworks.length &&  typeof model.artworks === 'string') {
    ...
    public get coverPicture() { return this._coverPicture }
    ...
    public translate(language: Language) {
        ...
        if (this.artists && this.artists.length && this.artists[0] instanceof Artist) {
            this.artists.forEach((x:Artist) => x.translate(language))
        }
        if (this.artworks && this.artworks.length && this.artworks[0] instanceof Artwork) {
